...
container1=$(buildah from "${1:-fedora:41}")
...
buildah config --label org.label-schema.description="Fedora 41" "$container1"
...
# buildah commit "$container1" ${2:-docker.io/jarrydk/fedora-updates:41}
buildah commit "$container1" ${2:-localhost/jarrydk/fedora-updates:41}