...
container1=$(buildah from "${1:-fedora:38}")
...
buildah config --label org.label-schema.description="Fedora 38" "$container1"
...
buildah config --label org.label-schema.version="38" "$container1"
...
# buildah commit "$container1" ${2:-docker.io/jarrydk/fedora-updates:38}
buildah commit "$container1" ${2:-jarrydk/fedora-updates:38}