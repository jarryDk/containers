...
buildah config --label org.label-schema.name="fedora-minimal-37-openjdk" "$container1"
buildah config --label org.label-schema.description="Fedora Minimal 37 - OpenJdk - ${OPEN_JDK_VERSION}" "$container1"
...
# buildah commit "$container1" ${2:-docker.io/jarrydk/fedora-minimal-37-adoptium-openjdk:20}
buildah commit "$container1" ${2:-jarrydk/fedora-minimal-37-adoptium-openjdk:20}
