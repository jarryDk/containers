...
container1=$(buildah from "${1:-jarry-fedora-openjdk:16}")
...
make images
if [ ! -d /opt/java/openjdk-next ]; then
	mkdir -p /opt/java/openjdk-next
fi
cp -R /opt/java/jdk/build/linux-x86_64-server-release/images/jdk /opt/java/openjdk-next/jdk-next
EOF
