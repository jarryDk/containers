...
# container1=$(buildah from "${1:-docker.io/jarrydk/fedora-updates:37}")
container1=$(buildah from "${1:-jarrydk/fedora-updates:37}")
...
buildah config --label org.label-schema.name="fedora-37-openjdk" "$container1"
buildah config --label org.label-schema.description="Fedora 37 - OpenJdk - ${OPEN_JDK_VERSION}" "$container1"
...
# buildah commit "$container1" ${2:-docker.io/jarrydk/fedora-37-adoptium-openjdk:20}
buildah commit "$container1" ${2:-jarrydk/fedora-37-adoptium-openjdk:20}

echo "----------------------------"
echo "------   TESTING  ----------"
echo "----------------------------"
podman run -it --rm=true \
    jarrydk/fedora-37-adoptium-openjdk:20 \
    java -version